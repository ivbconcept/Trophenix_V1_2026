/*
  # Fix auto profile trigger - Ajouter support des sponsors

  1. Modifications
    - Ajout du cas 'sponsor' dans le trigger
    - Création automatique du profil sponsor après confirmation
    - Ajout de logs pour debugging

  2. Sécurité
    - Pas de changement RLS
    - Fonction SECURITY DEFINER maintenue
*/

-- Recréer la fonction avec support des sponsors
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM profiles WHERE id = NEW.id) THEN
    ...
    ELSIF user_type_value = 'sponsor' THEN
      ...
      VALUES (
        NEW.id,
        profile_data_value->>'sponsor_name',
        profile_data_value->>'sponsor_type',
...
'Crée automatiquement les profils utilisateur (athlete, company, sponsor) après confirmation de l''email. 
