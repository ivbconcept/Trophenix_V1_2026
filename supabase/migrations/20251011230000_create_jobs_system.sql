...
CREATE TABLE IF NOT EXISTS job_applications (
  ...
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now(),
  UNIQUE(job_offer_id, athlete_id) -- Un athlète ne peut postuler qu'une fois par offre
);
...
CREATE TABLE IF NOT EXISTS saved_jobs (
  ...
  UNIQUE(athlete_id, job_offer_id) -- Un athlète ne peut sauvegarder qu'une fois une offre
);
...
-- Companies peuvent créer des offres
...
-- Companies peuvent voir et modifier leurs propres offres
...
-- Athletes peuvent voir les offres publiées
...
-- Athletes peuvent voir leurs propres candidatures
CREATE POLICY "Athletes can view own applications"
  ON job_applications FOR SELECT
  TO authenticated
  USING (athlete_id = auth.uid());

-- Athletes peuvent modifier leurs candidatures (seulement si pending)
...
-- Companies peuvent voir les candidatures pour leurs offres
...
-- Companies peuvent modifier le statut des candidatures
...
-- Athletes peuvent sauvegarder des offres
...
-- Athletes peuvent voir leurs offres sauvegardées
CREATE POLICY "Athletes can view own saved jobs"
  ON saved_jobs FOR SELECT
  TO authenticated
  USING (athlete_id = auth.uid());

-- Athletes peuvent supprimer leurs sauvegardes
